    ...
    if request.user.is_authenticated:
        ...
        if len(request.POST) > 0:
            if 'name_rgn' not in request.POST or 'abbrev_rgn' not in request.POST or 'img_rgn' not in request.POST:
                ...
                rgn_context = {"list_regions": list_regions, 'title': 'LISTE DES REGIONS', 'error': error}
...
def plotMap(request, id_sch):
    if request.user.is_authenticated:
        ...
        m = folium.Map([arr.latitude_arr, arr.longitude_arr], zoom_start=12, API_key='your.API.key')

        for i in schools:
            folium.Marker([i.latitude_sch, i.longitude_sch], popup='<i>' + i.name_sch + '</i>',
                          tooltip=i.name_sch).add_to(m)
...
def allSchoolVp(request, id_arr):
    # Detail.objects.get(infrastruture=infrastruture.id_inf).year_dtl
    schools = School.objects.filter(arrondissement=id_arr)
    regions = Region.objects.all()
    arrs = Arrondissement.objects.all()
    i = -1
    wel_context = {'title': 'Détails sur l\'établissemenr',
                   'schools': schools, 'arrs': arrs, 'regions': regions}
...
def publicMapSchool(request, id_sch):
    ...
    for i in schools:
        folium.Marker([i.latitude_sch, i.longitude_sch], popup='<i>' + i.name_sch + '</i>', tooltip=i.name_sch).add_to(
            m)

    m = m._repr_html_()  # updated
    context = {'my_map': m, 'arrs': arrs, 'title': 'Localisation', 'school': school, 'regions': regions}

    return render(request, 'public/map_school.html', context)
...
def ChoiceSchool(request):
    ...
    regions = Region.objects.all()
    arrs = Arrondissement.objects.all()
    choice_context = {'title': "ORIENTATION : Choix d'une école ", 'schools': schools, 'arrs': arrs, regions: 'regions'}
...
def statRgn(request, id_rgn):
    ...
    wel_context = {'title': 'Statistiques', 'regions': regions, 'schools': schools,
                   'arrondissements': arrondissements, 'region': region, 'departments': departments, 'arrs': arrs}
